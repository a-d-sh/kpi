...
export const POST = async (req, res) => {
  ...
  try {
    await writeFile(path.join(process.cwd(), 'public/files/kpi221/fileone/' + filenameone), bufferone)

    const res = await prisma.kpi221.create({
      data: {
        fileone: filenameone,
        title: title,
        link: link,
...
export const GET = async (req, res) => {
  ...
  const kpi221 = await prisma.kpi221.findMany({
    where: {
      userId: user.id
    }
  })
