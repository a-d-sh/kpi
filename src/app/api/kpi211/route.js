...
export const POST = async (req, res) => {
  ...
  try {
    await writeFile(path.join(process.cwd(), 'public/files/kpi211/fileone/' + filenameone), bufferone)

    const res = await prisma.kpi211.create({
      data: {
        fileone: filenameone,
        title: title,
        link: link,
...
export const GET = async (req, res) => {
  ...
  const kpi211 = await prisma.kpi211.findMany({
    where: {
      userId: user.id
    }
  })
