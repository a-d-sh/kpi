...
export const POST = async (req, res) => {
  ...
  try {
    await writeFile(path.join(process.cwd(), 'public/files/kpi241/fileone/' + filenameone), bufferone)

    const res = await prisma.kpi241.create({
      data: {
        fileone: filenameone,
        title: title,
        publisher: publisher,
...
export const GET = async (req, res) => {
  ...
  const kpi241 = await prisma.kpi241.findMany({
    where: {
      userId: user.id
    }
  })
