...
export const POST = async (req, res) => {
  ...
  try {
    await writeFile(path.join(process.cwd(), 'public/files/kpi151/fileone/' + filenameone), bufferone)

    const res = await prisma.kpi151.create({
      data: {
        fileone: filenameone,
        title: title,
        place: place,
...
export const GET = async (req, res) => {
  ...
  const kpi151 = await prisma.kpi151.findMany({
    where: {
      userId: user.id
    }
  })
